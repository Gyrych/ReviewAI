...
export function makeStructuredReviewHandler(usecase: MultiModelReviewUseCase) {
  return async function handler(req: Request, res: Response) {
    try {
      ...
      const models = Array.isArray(body.models) ? body.models : (() => { try { return JSON.parse(body.models||'[]') } catch { return [] } })()
      ...
      if (!apiUrl || !models || models.length === 0) return res.status(400).json({ error: 'apiUrl and models required' })

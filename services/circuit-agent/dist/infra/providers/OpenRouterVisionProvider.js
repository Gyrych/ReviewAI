...
export class OpenRouterVisionProvider {
    ...
    async recognizeSingle(image, prompt, model) {
        ...
        if (!resp.ok)
            throw new Error(`vision upstream ${resp.status}`);
        // 简化解析：尝试从文本中提取 JSON
    ...
    async recognizeMultiPass(images, model, passes) {
    ...
    async consolidate(results, model) {
        const body = {
            model,
            messages: [
        ...
        if (!resp.ok)
            throw new Error(`consolidation upstream ${resp.status}`);
        const parsed = this.extractJson(resp.text);
        if (!parsed)
            return { components: [], nets: [] };
        return this.normalize(parsed);
    ...
    normalize(raw) {
        ...
        if ((!nets || nets.length === 0) && Array.isArray(raw.connections)) {
            ...
            for (const c of raw.connections) {
                const pins = [];
                try {
                    if (c?.from?.componentId && c?.from?.pin)
                        pins.push(`${c.from.componentId}.${c.from.pin}`);
                }
                catch { }
                try {
                    if (c?.to?.componentId && c?.to?.pin)
                        pins.push(`${c.to.componentId}.${c.to.pin}`);
                }
                catch { }
                if (pins.length >= 2)
                    tmp.push({ netId: `N${idx++}`, connectedPins: pins.map(p => { const [cid, pin] = p.split('.'); return { componentId: cid, pin }; }) });
            }
            nets = tmp;
        }

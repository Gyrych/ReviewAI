...
let progressStore = new ProgressMemoryStore();
(async () => {
    try {
        ...
        console.log('[circuit-fine-agent] Progress store: Memory (fallback)', e?.message || '');
    }
})();
...
app.get(`${BASE_PATH}/system-prompt`, (req, res) => {
    try {
        ...
        if (!p)
            return res.status(404).type('application/json').send(JSON.stringify({ error: 'system prompt not found' }));

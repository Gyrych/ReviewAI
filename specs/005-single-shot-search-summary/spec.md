...
- **FR-007**: 当响应无引用注解时，系统必须仍返回内容并标记“无引用”，同时记录可观测日志以便分析。
...
- **FR-017**: 必须删除所有旧多轮模式相关代码（后端流程、控制分支、开关与配置项、测试与脚本、CI 检查、环境变量），并清理调用点与死代码。
- **FR-018**: 必须同步前端页面与逻辑，移除多轮模式 UI/状态/请求路径，确保“搜索+摘要”单轮的展示（回答与引用）与交互一致。
- **FR-019**: 必须同步存储与数据层，删除多轮模式相关的存储字段、索引与读写逻辑，迁移或兼容旧数据，避免遗留字段导致的运行风险。
...
- 删除多轮模式：不仅隐藏开关，也要删除后端流程与前端逻辑，避免“暗路径”。
...
- **Citation**: 引用条目（url、title、content?、start_index、end_index）。
...
- **SC-005**: 升级后对上游接口无破坏性影响（若新增字段，保持向后兼容），相关集成用例全部通过。

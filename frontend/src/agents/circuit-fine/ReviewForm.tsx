/*
功能：电路评审表单（委员会架构）
用途：封装与 ReviewForm 的集成，提供保存/列表/导入导出入口。
参数：
- 无显式 props
返回：
- React 组件
示例：
// <CircuitFineReviewForm />
*/
...
export default function CircuitFineReviewForm(props: any) {
  ...
  async function openSessionList() {
    try {
      const base = props.agentBaseUrl || '/api/v1/circuit-fine-agent'
  ...
  return (
    <div className="relative">
      ...
      <ReviewForm ref={ref} {...props} initialMode="fine" />
    </div>
  )
}

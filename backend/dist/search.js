...
function cacheKey(q, provider, topN) {
...
async function webSearch(query, opts) {
    ...
    if (cached && (now - cached.ts) < DEFAULT_TTL_MS)
        return cached.data;
    let results = [];
    ...
    try {
        ...
        if (provider === 'bing' || (opts && opts.apiKey || process.env.BING_API_KEY)) {
            ...
            if (keyVal) {
                providerUsed = 'bing';
                const url = new url_1.URL('https://api.bing.microsoft.com/v7.0/search');
                url.searchParams.set('q', query);
                url.searchParams.set('count', String(topN));
                const resp = await (0, node_fetch_1.default)(url.toString(), { headers: { 'Ocp-Apim-Subscription-Key': keyVal }, timeout: 10000 });
                if (resp.ok) {
                    const j = await resp.json();
                    if (j.webPages && Array.isArray(j.webPages.value)) {
                        results = j.webPages.value.slice(0, topN).map((v) => ({ title: v.name || '', url: v.url || '', snippet: v.snippet || v.snippet || '' }));
                    }
                }
            }
        }
        ...
        if (results.length === 0) {
            ...
            try {
                ...
                const resp = await (0, node_fetch_1.default)(searchUrl, {
                    headers: {
                ...
                if (resp && resp.ok) {
                    ...
                    if (results.length === 0) {
                        try {
                            ...
                            (0, logger_1.logInfo)('search.html.snippet', { provider: 'duckduckgo_html', query, snippet });
                        }
                        catch (e) {
                            // 忽略日志写入错误
                        }
                    ...
                    let snippet = '';
                    try {
                        snippet = (await resp.text()).slice(0, 1024);
                    }
                    catch {
                        snippet = 'could not read body';
                    }
                    (0, logger_1.logError)('search.html.fetch_failed', { provider: 'duckduckgo_html', url: searchUrl, http_status: resp.status, snippet });
                }
                ...
                if (results.length === 0) {
                    try {
                        ...
                        const pResp = await (0, node_fetch_1.default)(proxyUrl, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 15000 });
                        if (pResp && pResp.ok) {
                                ...
                                try {
                                    ...
                                    while ((m = urlRegex.exec(pHtml)) !== null) {
                                        ...
                                        if (!u.startsWith('http'))
                                            continue;
                                        try {
                                            const host = new url_1.URL(u).hostname.toLowerCase();
                                            ...
                                            if (u.toLowerCase().endsWith('.pdf'))
                                                score += 5;
                                            if (preferred.some(p => host.includes(p)))
                                                score += 3;
                            ...
                            if (results.length === 0) {
                                ...
                                (0, logger_1.logInfo)('search.html.proxy_snippet', { provider: 'jina_proxy', query, snippet });
                            }
                            else {
                                (0, logger_1.logInfo)('search.html.proxy_success', { provider: 'jina_proxy', query, count: results.length });
                            }
                        }
                        else if (pResp) {
                            let snippet = '';
                            try {
                                snippet = (await pResp.text()).slice(0, 1024);
                            }
                            catch {
                                snippet = 'could not read body';
                            }
                            (0, logger_1.logError)('search.proxy.fetch_failed', { provider: 'jina_proxy', url: proxyUrl, http_status: pResp.status, snippet });
                        }
                    }
                    catch (e) {
                        const errMsg = e && e.message ? e.message : String(e);
                        (0, logger_1.logError)('search.proxy.exception', { provider: 'jina_proxy', query, error: errMsg });
                    }
                }
            }
            catch (e) {
                ...
                (0, logger_1.logError)('search.html.exception', { provider: 'duckduckgo_html', query, error: errMsg, stack });
            }
            // 如果HTML搜索也没有结果，使用Instant Answer API
            if (results.length === 0) {
                const url = new url_1.URL('https://api.duckduckgo.com/');
                url.searchParams.set('q', query);
                url.searchParams.set('format', 'json');
                url.searchParams.set('no_html', '1');
                url.searchParams.set('skip_disambig', '1');
                const resp = await (0, node_fetch_1.default)(url.toString(), { timeout: 10000 });
                if (resp.ok) {
                    const j = await resp.json();
                            ...
                            if (t.Text && t.FirstURL) {
                                ...
                                for (const tt of t.Topics) {
                                    if (tt.Text && tt.FirstURL)
                                        results.push({ title: tt.Text.split(' - ')[0], url: tt.FirstURL, snippet: tt.Text });
                                }
                            }
    ...
    const out = { query, provider: providerUsed, results, fetchedAt: new Date().toISOString() };
    try {
